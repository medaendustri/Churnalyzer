...
async function getPosts(page = 1): Promise<PostsResponse | null> {
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}/api/posts?page=${page}&limit=12`,
      {
        cache: "no-store",
      },
    )
...
export default async function PostsPage({ searchParams }: { searchParams: { page?: string } }) {
    ...
    return (
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-900">Unable to load case studies</h1>
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
      {/* Header */}
      <div className="mb-16">
        <h1 className="text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">All Case Studies</h1>
        <p className="mt-4 text-lg text-slate-600 max-w-2xl">
          Comprehensive analysis of business failures across industries. Learn from the mistakes of others to build
        ...
        <div className="mt-6 flex items-center gap-4">
          ...
          <Link href="/categories" className="text-sm text-slate-600 hover:text-slate-900 underline underline-offset-4">
            Browse by category
          </Link>
        </div>
      </div>
        ...
        <div className="mt-16 flex items-center justify-center gap-4">
          {pagination.page > 1 && (
            <Button asChild variant="outline">
              <Link href={`/posts?page=${pagination.page - 1}`}>← Previous</Link>
            </Button>
          )}
